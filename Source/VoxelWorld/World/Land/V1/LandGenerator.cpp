...
namespace LandGenerator_V1
{
    ...
    float LandGenerator::BaseHeight(BiomeType type)
    {
    ...
    float LandGenerator::VariHeight(BiomeType type)
    {
        switch(type)
        {
        case BiomeType::Ocean:
    ...
    void LandGenerator::MakeOcean(Chunk *ck, int x, int z, int h)
    {
    ...
    void LandGenerator::MakeField(Chunk *ck, int x, int z, int h)
    {
        ck->SetBlockType(x, 0, z, BlockType::Bedrock);
        for(int y = 1; y < h - 2; ++y)
    ...
    void LandGenerator::MakePlain(Chunk *ck, int x, int z, int h)
    {
        MakeField(ck, x, z, h);
    }

    void LandGenerator::MakeDesert(Chunk *ck, int x, int z, int h)
    {
        ck->SetBlockType(x, 0, z, BlockType::Bedrock);
        for(int y = 1; y <= h; ++y)
            ck->SetBlockType(x, y, z, BlockType::Sand);
    ...
    void LandGenerator::MakeHill(Chunk *ck, int x, int z, int h)
    {
        ck->SetBlockType(x, 0, z, BlockType::Bedrock);
        for(int y = 1; y <= h; ++y)
            ck->SetBlockType(x, y, z, BlockType::Stone);
